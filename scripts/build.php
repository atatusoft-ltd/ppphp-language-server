...
/**
 * Build UNC sources on the native filesystem. In addition to cmd.exe's UNC
 * limitation, Windows npm cannot create workspace junctions on a WSL share.
 * The managed copy excludes dependencies and build caches from the source host.
 */
function run_windows_unc_build(string $root, string $target): void
{
    $localData = getenv('LOCALAPPDATA');
    if ($localData === false || $localData === '') {
        throw new RuntimeException('LOCALAPPDATA is required for native Windows builds');
    }
    $parent = $localData . '/ppphp/windows-build';
    $staged = $parent . '/source-' . substr(hash('sha256', strtolower($root)), 0, 12);
    $marker = $staged . '/.ppphp-generated';
    if (is_dir($staged) && !is_file($marker)) {
        throw new RuntimeException("refusing to replace unmanaged build directory: {$staged}");
    }
    remove_generated_directory($staged, $parent);
    ensure_directory($staged);
    if (file_put_contents($marker, "Generated by scripts/build.php for a Windows UNC build.\n") === false) {
        throw new RuntimeException('could not mark the generated Windows source directory');
    }
    copy_windows_build_source($root, $staged);
    fwrite(STDOUT, "Building UNC source from the native Windows directory: {$staged}\n");
    run_command([PHP_BINARY, $staged . '/scripts/build.php', $target], $staged);
}

/** Copy source inputs, excluding generated files and host-specific dependencies. */
function copy_windows_build_source(string $source, string $destination, string $relative = ''): void
{
    $excluded = ['.git', '.codex', '.agents', '.idea', '.gradle', '.intellijPlatform',
        '.kotlin', 'node_modules', 'build', 'dist', 'target', 'coverage'];
    foreach (scandir($source) ?: [] as $name) {
        if ($name === '.' || $name === '..' || in_array($name, $excluded, true)) {
            continue;
        }
        $path = $source . '/' . $name;
        $child = ltrim($relative . '/' . $name, '/');
        if ($child === 'editors/zed/grammars' || str_ends_with($name, '.wasm')
            || str_ends_with($name, '.vsix') || is_link($path)) {
            continue;
        }
        if (is_dir($path)) {
            ensure_directory($destination . '/' . $name);
            copy_windows_build_source($path, $destination . '/' . $name, $child);
        } elseif (is_file($path)) {
            copy_required_file($path, $destination . '/' . $name);
        }
    }
}
...
function windows_command(string $executable, array $arguments): array
{
    ...
    // from the script's absolute path, including from a staged UNC checkout.
