...
The [Zed extension](../editors/zed/README.md) is implemented under `editors/zed`. Its pinned ++PHP Tree-sitter grammar supplies types, nested generics, typed locals, checked-error clauses, and editing structure without LSP or semantic-token settings. The shared server supplies compiler-backed semantics and edits. Configured commands and local development bundles remain available; normal installations use Zed's host-local Node.js runtime and an exact-version GitHub release asset. Downloads and cached bundles are checked against the release's SHA256SUMS, and partial downloads are discarded. Linux and native Windows checks cover the adapter, parser, queries, and installer. Standalone release artifacts and monorepo registry instructions are prepared; publishing the server asset, completing clean-profile/platform smoke tests, and submitting the registry entry remain release steps.
