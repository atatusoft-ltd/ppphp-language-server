...
Type import actions are editor-neutral `refactor.rewrite` actions, not diagnostic-only quick fixes. A client may opt into `initializationOptions.typeActions.groupedImports` to receive one import action with candidate edits in `data.ppphp.choices`; otherwise it receives individual candidate actions. `classCreation` advertises a native creation UI: only clients implementing that action opt in (currently PhpStorm). The native adapter reuses the declaration dialog and platform workspace-edit handling; the server owns import selection, alias safety, sorting, and compiler-backed exclusion of already-resolved scoped symbols. Fully qualified references remain eligible without a diagnostic. Versioned action metadata prevents applying a chooser result after the source changes.

Qualified type names without a leading `\` are also eligible for **Use import**. Their target comes from the compiler's `type:<fqn>` symbol identity, not a textual catalog match: namespace-relative names and namespace aliases must retain their meaning. The catalog supplies canonical display spelling and the existing import planner handles aliases, ordering, and collisions. Unresolved names, unavailable resolution, and non-type symbols do not produce speculative shortening edits. Absolute names keep their existing catalog-backed path without an extra compiler query.

The adapter targets the minimum IDE API declared in the plugin build configuration. Newer IDEs forward the `LspClient` customization overload to the older `LspServer` overload, allowing one binary to support the tested compatibility range. Plugin verification reports that overload as deprecated; migrate it when raising the minimum IDE baseline rather than introducing reflective dispatch. Type-context recognition uses a linear fragment scanner instead of nested identifier repetitions, which could otherwise stall both import actions and completion through catastrophic backtracking.

`packages/language-server` communicates over standard input/output or VS Code IPC. It supports incremental document synchronization. Diagnostics use `editor:diagnostics` version 1 with current unsaved contents and other open project-buffer overlays, never source-file writes. A 50ms debounce coalesces changes; save/configuration changes refresh immediately. The most recently edited document is checked first, with other open documents still checked afterward because an overlay can change their meaning. Analysis rounds are single-flight and use immutable snapshots. Every edit or close invalidates the whole overlay generation immediately and cancels its compiler subprocess. The next round waits for that subprocess to finish closing, so cancellation does not create overlapping compiler jobs. Neither a stale target nor a changed dependency can publish an older result or an obsolete cancellation warning. Definition and semantic-token requests also include the current unsaved document. See [diagnostic latency](diagnostic-performance.md) for measurements and remaining compiler-side costs.

Live diagnostic coverage is `compilerCore` with `supplemental:false`: it does not include optional PHPStan findings supplied by saved-project `ppphp check`. `analysis.fullParity` describes the compiler capability catalog, not supplemental-analyzer parity. Clients log this distinction. An old compiler lacking the endpoint produces an actionable unavailable message; the server does not silently analyze saved text as if it were the current buffer. Requests enforce the compiler's per-buffer, aggregate-content and document-count bounds. The compiler strictly validates target ownership and ignores well-formed, non-owned context buffers (such as vendor, output, or stub files); the LSP does not reconstruct source-root rules or inject excluded declarations.
...
The PhpStorm plugin declares its compatibility baseline in the build configuration, uses bundled PHP tokenization for shallow PSI and native PHP lexical highlighting, and maps LSP semantic roles to PhpStorm's PHP color keys for parser-dependent PHP and ++PHP constructs. It registers the `.ppphp` language and starts the bundled server with a separately installed Node.js runtime compatible with the server's package manifest.
...
The ++PHP code-style provider delegates option definitions, labels, previews, and defaults to PhpStorm's bundled PHP provider, then remaps PHP-specific formatter fields onto a separate ++PHP settings object. PHPDoc, code-conversion, and code-generation panels use a private settings bridge so their controls remain native without modifying the PHP scheme. Compatibility tests require the ++PHP field contract to cover PhpStorm's PHP contract. The formatting model applies structural indentation, spacing, brace, and blank-line settings over the shallow PSI; quoted strings and heredocs are opaque. Generated class-family declarations consume the same common settings, and the canonical ++PHP default is a next-line class brace.

Matching controls are not proof of formatter parity. Differential tests format the same source with PHP and ++PHP under identical common/custom settings, covering the indentation preview, expression continuations, nested closures, and the distinct operator controls. Tests also verify idempotence and protect literal contents and the nullsafe operator, even where the baseline PHP formatter itself splits that operator with non-default spacing. Full PHP formatting parity remains open: the complete PHP spacing preview still exposes differences in switch/case bodies, casts, ternary/operator contexts, and mixed PHP/HTML. Closing these gaps requires structural context and end-to-end output tests, not more UI-only assertions or substituting a PHP-formatted preview for the actual ++PHP formatter output.
