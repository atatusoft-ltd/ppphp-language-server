...
The adapter targets the supported 2025.2 LSP API. PhpStorm 2026.2 forwards its newer `LspClient` customization overload to the older `LspServer` overload, so the same binary works on both baselines. Plugin verification reports that overload as deprecated; migrate it when raising the minimum IDE version rather than introducing reflective dispatch. Type-context recognition uses a linear fragment scanner instead of nested identifier repetitions, which could otherwise stall both import actions and completion through catastrophic backtracking.
